...
registerMooseObject("RedbackApp", RedbackMechMaterialHOHard);

/**
 * RedbackMechMaterialHO handles a high order material.
 */
InputParameters
RedbackMechMaterialHOHard::validParams()
{
  ...
  params.addParam<Real>("hardA", 0, " mechanical hardening parameter A value");
  params.addParam<Real>("hardB", 0, " mechanical hardening parameter B value");
  params.addParam<Real>("hardC", 0, " mechanical hardening parameter C value");
  params.addParam<Real>("hardD", 0, " mechanical hardening parameter D value");
  params.addParam<Real>("hardE", 0, " mechanical hardening parameter E value");
...
RedbackMechMaterialHOHard::RedbackMechMaterialHOHard(const InputParameters & parameters) :
    ...
    _hardening_mech_modulus(declareProperty<Real>("hardening_mech_modulus")),
    ...
    _hardA(getParam<Real>("hardA")),
    _hardB(getParam<Real>("hardB")),
    _hardC(getParam<Real>("hardC")),
    _hardD(getParam<Real>("hardD")),
    _hardE(getParam<Real>("hardE")),
...
void
RedbackMechMaterialHOHard::initQpStatefulProperties()
{
...
void
RedbackMechMaterialHOHard::computeQpStrain(const RankTwoTensor & Fhat)
{
...
void
RedbackMechMaterialHOHard::computeQpStress()
{
...
_hardening_mech_modulus[_qp]=_hardA * std::pow(1 - std::exp( - _hardB*(_hardening_variable_old[_qp]- _hardC )), _hardE ) - _hardD;
...
PROPS[5]=_cohesion;
PROPS[6]=_hardening_mech_modulus[_qp];
...
PROPS2[5]=_cohesion;
PROPS2[6]=_hardening_mech_modulus[_qp];
...
int RedbackMechMaterialHOHard::corsigma(int num1, int num2){
  int result;
...
int RedbackMechMaterialHOHard::cormoment(int num1, int num2){
  int result;
...
void RedbackMechMaterialHOHard::recupSigmaNew(RankTwoTensor & tens_new, Real* vect, int  ini){
  for (unsigned i = 0; i < 3; ++i){
...
void RedbackMechMaterialHOHard::recupMomentNew(RankTwoTensor & tens_new, Real* vect, int  ini){
...
void RedbackMechMaterialHOHard::computeQpElasticityTensor()
{
...
void
RedbackMechMaterialHOHard::returnMap(const RankTwoTensor & sig_old,
...
void
RedbackMechMaterialHOHard::getJac(const RankTwoTensor & sig,
                              const RankFourTensor & E_ijkl,
                              Real flow_incr,
                              Real sig_eqv,
                              Real pressure,
...
void
RedbackMechMaterialHOHard::computeRedbackTerms(RankTwoTensor & sig, Real q_y, Real p_y)
{
