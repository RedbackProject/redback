...
RedbackInertialForce::validParams()
{
  ...
  params.addParam<Real>("microinertia",1,"micro-inertia linked to the size of the grain");
  return params;
}

RedbackInertialForce::RedbackInertialForce(const InputParameters & parameters) :
    Kernel(parameters),
    ...
    _microinertia(getParam<Real>("microinertia"))
{}

Real
RedbackInertialForce::computeQpResidual()
{
  if (_dt == 0)
    return 0;
  else
  {
    ...
    return _test[_i][_qp] * _density[_qp] * _microinertia * (accel + vel * _eta*(1+_alpha) - _alpha*_eta*_vel_old[_qp]);
  }
}

Real
RedbackInertialForce::computeQpJacobian()
{
  if (_dt == 0)
    return 0;
  else
    return _test[_i][_qp] * _density[_qp] * _microinertia / (_beta * _dt * _dt) * _phi[_j][_qp]
      + _eta * (1+_alpha) * _test[_i][_qp] * _density[_qp] * _microinertia * _gamma / _beta / _dt * _phi[_j][_qp];
}
